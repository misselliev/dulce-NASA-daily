...
import { Segment, Image, Button, Icon, Header, Form } from 'semantic-ui-react';
...
const ImageCard: React.FC = () => {
    const daily: DailyProps = useSelector((state: any) => state.daily.daily, shallowEqual) || [];
    const error = useSelector((state: any) => state.daily.error, shallowEqual) || [];
    const [search, setSearch] = useState('');
    const [newDate, setDate] = useState('');
    const [startDate, setStartDate] = useState(new Date());
    const dispatch = useDispatch();

    const selectDay = (newDate: string) => {
